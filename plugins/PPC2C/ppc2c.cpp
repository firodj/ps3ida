...
bool GenerateRotate32(char *buff, int buffSize, const char *src, int leftShift, int rightShift,
                      unsigned int &mask) {
    // work out "rotate" part of the instruction
    if (leftShift == 0 && rightShift == 32 || leftShift == 32 && rightShift == 0) {
...
// generate string showing rotation or shifting within instruction
// returns:	true if string requires brackets if a mask is used, ie: (r4 << 2) & 0xFF
bool GenerateRotate64(char *buff, int buffSize, const char *src, int leftShift, int rightShift,
                      unsigned long long &mask) {
    // work out "rotate" part of the instruction
    if (leftShift == 0 && rightShift == 64 || leftShift == 64 && rightShift == 0) {
        // no rotation
        qsnprintf(buff, buffSize, "%s", src);
        return false;
    }
...
bool Rotate_iMask32(ea_t ea, char *buff, int buffSize, const char *leftRotate, int mb, int me) {
    // calculate the mask
    ...
    qsnprintf(rot_str, sizeof(rot_str), "(%s << %s) | (%s >> 32-%s)", g_RS, leftRotate, g_RS,
              leftRotate);
...
bool iRotate_iMask32(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    // calculate the mask
    // if no mask, then result is always 0
    unsigned int mask = GenerateMask32(mb, me);
    if (mask == 0) {
    ...
    char rot_str[G_STR_SIZE];
    bool brackets =
        GenerateRotate32(rot_str, sizeof(rot_str), g_RS, leftRotate, 32 - leftRotate, mask);
    if (mask == MASK32_ALLSET) {
        //		if(brackets)
        //			qsnprintf(buff, buffSize, "%s = (u32)(%s)", g_RA, rot_str);
        //		else
...
bool insert_iRotate_iMask32(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    // calculate the mask
    ...
    bool brackets =
        GenerateRotate32(rot_str, sizeof(rot_str), g_RS, leftRotate, 32 - leftRotate, rot_mask);
    ...
    if (mask == MASK32_ALLSET) {
        ...
        if (brackets)
            qsnprintf(buff, buffSize, "%s = (%s & ~%s) | (%s)", g_RA, g_RA, mask_str, rot_str);
        else
            qsnprintf(buff, buffSize, "%s = (%s & ~%s) | %s", g_RA, g_RA, mask_str, rot_str);
    } else {
        if (brackets)
            qsnprintf(buff, buffSize, "%s = (%s & ~%s) | ((%s) & %s)", g_RA, g_RA, mask_str,
                      rot_str, rot_mask_str);
        else
            qsnprintf(buff, buffSize, "%s = (%s & ~%s) | (%s & %s)", g_RA, g_RA, mask_str, rot_str,
                      rot_mask_str);
    }
    return true;
}

bool gen_rlwimi(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    return insert_iRotate_iMask32(ea, buff, buffSize, leftRotate, mb, me);
}

bool gen_rlwinm(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    return iRotate_iMask32(ea, buff, buffSize, leftRotate, mb, me);
}

bool gen_rlwnm(ea_t ea, char *buff, int buffSize, const char *leftRotate, int mb, int me) {
    return Rotate_iMask32(ea, buff, buffSize, leftRotate, mb, me);
}

// register rotate and immediate mask
bool Rotate_iMask64(ea_t ea, char *buff, int buffSize, const char *leftRotate, int mb, int me) {
    // calculate the mask
    // if no mask, then result is always 0
    ...
    // if all mask bits are set, then no need to use the mask
    char rot_str[G_STR_SIZE];
    qsnprintf(rot_str, sizeof(rot_str), "(%s << %s) | (%s >> 64-%s)", g_RS, leftRotate, g_RS,
              leftRotate);
    ...
    char mask_str[G_STR_SIZE];
    if (mask >> 32 == 0)
        qsnprintf(mask_str, sizeof(mask_str), "%s%X", (mask < 0xA) ? "" : "0x",
                  (unsigned long)mask);
    else
        qsnprintf(mask_str, sizeof(mask_str), "%s%X%08X", (mask < 0xA) ? "" : "0x",
                  (unsigned long)(mask >> 32), (unsigned long)mask);
...
// immediate rotate and immediate mask
bool iRotate_iMask64(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    // calculate the mask
    // if no mask, then result is always 0
    unsigned long long mask = GenerateMask64(mb, me);
    if (mask == 0) {
    ...
    char rot_str[G_STR_SIZE];
    bool brackets =
        GenerateRotate64(rot_str, sizeof(rot_str), g_RS, leftRotate, 64 - leftRotate, mask);
    if (mask == MASK64_ALLSET) {
        qsnprintf(buff, buffSize, "%s = %s", g_RA, rot_str);
        return true;
    }
...
// insert immediate rotate and immediate mask
bool insert_iRotate_iMask64(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    // calculate the mask
    // if no mask, then result is the untouched target register
    unsigned long long mask = GenerateMask64(mb, me);
    if (mask == 0) {
    ...
    bool brackets =
        GenerateRotate64(rot_str, sizeof(rot_str), g_RS, leftRotate, 64 - leftRotate, rot_mask);
    ...
    char rot_mask_str[G_STR_SIZE];
    if (rot_mask >> 32 == 0)
        qsnprintf(rot_mask_str, sizeof(rot_mask_str), "%s%X", (rot_mask < 0xA) ? "" : "0x",
                  (unsigned long)rot_mask);
    else
        qsnprintf(rot_mask_str, sizeof(rot_mask_str), "%s%X%08X", (rot_mask < 0xA) ? "" : "0x",
                  (unsigned long)(rot_mask >> 32), (unsigned long)rot_mask);
    //	unsigned long long not_mask = ~mask;
    //	char not_mask_str[G_STR_SIZE];
    //	if(not_mask>>32 == 0)
    //		qsnprintf(not_mask_str, sizeof(not_mask_str), "%s%X", (not_mask<0xA)?"":"0x",
    //(unsigned long)not_mask); 	else 		qsnprintf(not_mask_str,
    // sizeof(not_mask_str), "%s%X%08X", (not_mask<0xA)?"":"0x", (unsigned long)(not_mask>>32),
...
bool bc(ea_t ea, char *buff, int buffSize) {
    ...
    if (strcmp(condition_str, "lt") == 0)
        qstrncpy(condition_str, "less than", sizeof(condition_str));
    else if (strcmp(condition_str, "le") == 0)
        qstrncpy(condition_str, "less than or equal", sizeof(condition_str));
    else if (strcmp(condition_str, "eq") == 0)
        qstrncpy(condition_str, "equal", sizeof(condition_str));
    else if (strcmp(condition_str, "ge") == 0)
        qstrncpy(condition_str, "greater than or equal", sizeof(condition_str));
    else if (strcmp(condition_str, "gt") == 0)
        qstrncpy(condition_str, "greater than", sizeof(condition_str));

    else if (strcmp(condition_str, "nl") == 0)
        qstrncpy(condition_str, "not less than", sizeof(condition_str));
    else if (strcmp(condition_str, "ne") == 0)
        qstrncpy(condition_str, "not equal", sizeof(condition_str));
    else if (strcmp(condition_str, "ng") == 0)
        qstrncpy(condition_str, "not greater than", sizeof(condition_str));
    else if (strcmp(condition_str, "so") == 0)
        qstrncpy(condition_str, "summary overflow", sizeof(condition_str));
    else if (strcmp(condition_str, "ns") == 0)
        qstrncpy(condition_str, "not summary overflow", sizeof(condition_str));
    else if (strcmp(condition_str, "un") == 0)
        qstrncpy(condition_str, "unordered", sizeof(condition_str));
    else if (strcmp(condition_str, "nu") == 0)
        qstrncpy(condition_str, "not unordered", sizeof(condition_str));

    if ((BO & 0x1E) == 0x00) {
        // decrement the CTR, then branch if the decremented CTR(M:63) != 0 and CR(BI) == 0
        qsnprintf(buff, buffSize, "ctr--; if(ctr != 0 && %s is not %s) goto %s", cr_str,
                  condition_str, target_addr);
        ...
        qsnprintf(buff, buffSize, "ctr--; if(ctr != 0 && %s is %s) goto %s", cr_str, condition_str,
                  target_addr);
...
// rldcl RA, RS, RB, MB
bool gen_rldcl(ea_t ea, char *buff, int buffSize, const char *leftRotate, int mb, int me) {
    return Rotate_iMask64(ea, buff, buffSize, leftRotate, mb, me);
}

// Rotate Left Double Word then Clear Right
// rldcr RA, RS, RB, MB
bool gen_rldcr(ea_t ea, char *buff, int buffSize, const char *leftRotate, int mb, int me) {
    return gen_rldcl(ea, buff, buffSize, leftRotate, me, mb);
}

// Rotate Left Double Word Immediate then Clear
// rldic RA, RS, SH, MB
bool gen_rldic(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    return iRotate_iMask64(ea, buff, buffSize, leftRotate, mb, me);
}

// Rotate Left Double Word Immediate then Clear Left
// rldicl RA, RS, SH, MB
bool gen_rldicl(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    return iRotate_iMask64(ea, buff, buffSize, leftRotate, mb, me);
}

// Rotate Left Double Word Immediate then Clear Right
// rldicr RA, RS, SH, ME
bool gen_rldicr(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
    return iRotate_iMask64(ea, buff, buffSize, leftRotate, mb, me);
}

// Rotate Left Double Word Immediate then Mask Insert
// rldimi RA, RS, SH, MB
bool gen_rldimi(ea_t ea, char *buff, int buffSize, int leftRotate, int mb, int me) {
...
bool PPCAsm2C(ea_t ea, char *buff, int buffSize) {
    ...
    *g_opnd_s3         = 0;
    ...
    if (qstrcmp(g_mnem, "bc") == 0)
        return bc(ea, buff, buffSize);
    // clear
    else if (qstrcmp(g_mnem, "clrlwi") == 0)
        return clrlwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "clrrwi") == 0)
        return clrrwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "clrlslwi") == 0)
        return clrlslwi(ea, buff, buffSize);
    // extract
    else if (qstrcmp(g_mnem, "extlwi") == 0)
        return extlwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "extrwi") == 0)
        return extrwi(ea, buff, buffSize);
    // insert
    else if (qstrcmp(g_mnem, "inslwi") == 0)
        return inslwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "insrwi") == 0)
        return insrwi(ea, buff, buffSize);
    // rotate and mask
    else if (qstrcmp(g_mnem, "rlwimi") == 0)
        return rlwimi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rlwinm") == 0)
        return rlwinm(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rlwnm") == 0)
        return rlwnm(ea, buff, buffSize);
    // rotate
    else if (qstrcmp(g_mnem, "rotlw") == 0)
        return rotlw(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rotlwi") == 0)
        return rotlwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rotrwi") == 0)
        return rotrwi(ea, buff, buffSize);
    // shift
    else if (qstrcmp(g_mnem, "slwi") == 0)
        return slwi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "srwi") == 0)
        return srwi(ea, buff, buffSize);
    ...
    // clear
    else if (qstrcmp(g_mnem, "clrldi") == 0)
        return clrldi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "clrrdi") == 0)
        return clrrdi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "clrlsldi") == 0)
        return clrlsldi(ea, buff, buffSize);
    // extract
    else if (qstrcmp(g_mnem, "extldi") == 0)
        return extldi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "extrdi") == 0)
        return extrdi(ea, buff, buffSize);
    // insert
    else if (qstrcmp(g_mnem, "insrdi") == 0)
        return insrdi(ea, buff, buffSize);
    // rotate
    else if (qstrcmp(g_mnem, "rotld") == 0)
        return rotld(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rotldi") == 0)
        return rotldi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rotrdi") == 0)
        return rotrdi(ea, buff, buffSize);
    // rotate and mask
    else if (qstrcmp(g_mnem, "rldcl") == 0)
        return rldcl(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rldcr") == 0)
        return rldcr(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rldic") == 0)
        return rldic(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rldicl") == 0)
        return rldicl(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rldicr") == 0)
        return rldicr(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "rldimi") == 0)
        return rldimi(ea, buff, buffSize);
    // shift
    else if (qstrcmp(g_mnem, "sldi") == 0)
        return sldi(ea, buff, buffSize);
    else if (qstrcmp(g_mnem, "srdi") == 0)
        return srdi(ea, buff, buffSize);
...
static bool has_function(ea_t address) {
...
static bool has_instruction(Function &func, ea_t address) {
...
static bool parse_instruction(Function &func, ea_t ea) {
    Instruction ins;
    ...
    *buffer = 0;
    ...
    /*DEBUG ("Instruction at %a is : '%s' - '%s' - '%s' - '%s' - '%s' - '%s'\n", ea,
       ins.name.c_str(), ins.operands[0].c_str(), ins.operands[1].c_str(), ins.operands[2].c_str(),
       ins.operands[3].c_str(), ins.operands[4].c_str());*/
    func.instructions.push_back(ins);

    return true;
}

static bool parse_function(ea_t address, bool recursive = true) {
    func_t *p_func = NULL;
    bool success   = true;
    Function func;
...
break_loop2:
    ...
    while (modified) {
        ...
        for (; iter1 != instructions.end(); iter1++) {
            ...
            for (bool ok = xb.first_from(ea, XREF_ALL); ok && xb.iscode; ok = xb.next_from()) {
                // DEBUG("First xref from %a to %a : %s (%d)\n", xb.from, xb.to, xb.type == fl_F?
                // "Flow" : xb.type == fl_JN? "Jump near" : xb.type == fl_JF ? "Jump far" : xb.type
                // == fl_CN ? "Call near" : xb.type == fl_CF ? "Call far" : "unknown flow",
                // xb.type);
        ...
        for (; iter2 != instructions.end(); iter2++) {
            ...
            for (bool ok = xb.first_from(ea, XREF_ALL); ok && xb.iscode; ok = xb.next_from()) {
                ...
                if ((xb.type == fl_JN || xb.type == fl_JF) &&
                    instructions.find(xb.to) == instructions.end()) {
                    instructions.insert(xb.to);
                    modified = true;
                    goto break_loop2;
                }
    ...
    for (iter1 = instructions.begin(); success && iter1 != instructions.end(); iter1++) {
        ...
        for (bool ok = xb.first_from(ea, XREF_ALL); ok && xb.iscode; ok = xb.next_from()) {
            // DEBUG("Third xref from %a to %a : %s (%d)\n", xb.from, xb.to, xb.type == fl_F? "Flow"
            // : xb.type == fl_JN? "Jump near" : xb.type == fl_JF ? "Jump far" : xb.type == fl_CN ?
            // "Call near" : xb.type == fl_CF ? "Call far" : "unknown flow", xb.type);
...
static void generate_prototype(Function &func) {
...
static bool generate_functions() {
    list<Function>::iterator it;
    for (it = functions.begin(); it != functions.end(); it++) {
        Function func = *it;
...
void idaapi PluginMain(int param) {
    ...
    msg("Found %d functions in : %2d:%3d\n", functions.size(), (int)diffms / 1000,
        (int)diffms % 1000);
}

const char G_PLUGIN_COMMENT[] = "PPC To C Converter";
const char G_PLUGIN_HELP[] =
    "This plugin converts a function in PPC instructions into their relevant C code.\n";
...
plugin_t PLUGIN = {
    ...
    // strings
    (char *)
        G_PLUGIN_COMMENT,  // long comment about the plugin (may appear on status line or as a hint)
