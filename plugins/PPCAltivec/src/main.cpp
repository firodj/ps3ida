 ...
 *	well-known PowerPC implementations include support for Altivec (such as the Apple G4/G5
 *range, or the majority of next generation game consoles). Fortunately IDA Pro supports the concept
 *of extension modules that can add support for non-standard instructions, so this extension adds
 ...
 *	you can disable/re-enable the plugin by using the entry in the Edit/Plugins menu. If you
 *want the plugin to be disabled on load, you will have to edit this source code. Change the value
 *of g_HookState to 'kDisabled' and rebuild.
 *
 *
 *	NOTES
 *	-----
 *
 *	The versions of ppc.w32 and ppc64.w64 that were in initial distributions of IDA Pro 4.8
 *contain a flaw that will trigger an illegal read of memory when used with this extension. If you
 *happen to encounter crashes in these modules, I would recommend that you contact Data Rescue,
 ...
 *									Correction to operand order
 *for vmaddfp. Now handles initial analysis without any additional hassle. Added support for Altivec
 *opcodes with 4 parameters.
 *
 ...
 *									extension module created by
 *HyperIris. Also incldued support for SPRG names for PS3 as added by Tridentsx.
 *
 ***************************************************************************************************/
...
char g_pluginName[] = "PowerPC Altivec Extension " PPCALTIVEC_VERSION
                      " with support for VMX128, Xbox360(Xenon), PS3(CellBE) and GC/WII(Gekko).";
char g_pluginHelp[] =
    "This plugin enables recognition of many extra processor specific instructions\n"
    "when using IDA Pro's PowerPC processor module.\n"
    "The added instructions support Altivec, VMX128, Xbox360(Xenon), PS3(CellBE) and "
    "GC/WII(Gekko).\n";
...
altivec_opcode g_altivecOpcodes[] = {
    ...
    {altivec_vcmpbfp_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Bounds Floating Point (set CR6)"},
    {altivec_vcmpeqfp,
     "vcmpeqfp",
     VXR(4, 198, 0),
     VXR_MASK,
    ...
    {altivec_vcmpeqfp_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Equal-to Floating Point (set CR6)"},
    {altivec_vcmpequb,
     "vcmpequb",
     VXR(4, 6, 0),
     VXR_MASK,
    ...
    {altivec_vcmpequb_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Equal-to Unsigned Byte (set CR6)"},
    {altivec_vcmpequh,
     "vcmpequh",
     VXR(4, 70, 0),
     VXR_MASK,
    ...
    {altivec_vcmpequh_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Equal-to Unsigned Half Word (set CR6)"},
    {altivec_vcmpequw,
     "vcmpequw",
     VXR(4, 134, 0),
     VXR_MASK,
    ...
    {altivec_vcmpequw_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Equal-to Unsigned Word (set CR6)"},
    {altivec_vcmpgefp,
     "vcmpgefp",
     VXR(4, 454, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgefp_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than-or-Equal-to Floating Point (set CR6)"},
    {altivec_vcmpgtfp,
     "vcmpgtfp",
     VXR(4, 710, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtfp_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Floating Point (set CR6)"},
    {altivec_vcmpgtsb,
     "vcmpgtsb",
     VXR(4, 774, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtsb_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Signed Byte (set CR6)"},
    {altivec_vcmpgtsh,
     "vcmpgtsh",
     VXR(4, 838, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtsh_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Signed Half Word (set CR6)"},
    {altivec_vcmpgtsw,
     "vcmpgtsw",
     VXR(4, 902, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtsw_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Signed Word (set CR6)"},
    {altivec_vcmpgtub,
     "vcmpgtub",
     VXR(4, 518, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtub_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Unsigned Byte (set CR6)"},
    {altivec_vcmpgtuh,
     "vcmpgtuh",
     VXR(4, 582, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtuh_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Unsigned Half Word (set CR6)"},
    {altivec_vcmpgtuw,
     "vcmpgtuw",
     VXR(4, 646, 0),
     VXR_MASK,
    ...
    {altivec_vcmpgtuw_c,
     ...
     VXR_MASK,
     {VD, VA, VB},
     "Vector Compare Greater-Than Unsigned Word (set CR6)"},
    {altivec_vctsxs,
     "vctsxs",
     VX(4, 970),
     VX_MASK,
    ...
    {vmx128_vmaddcfp128,
     ...
     VX128(5, 272),
     VX128_MASK,
     {VD128, VA128, VS128, VB128},
     ""},
    {vmx128_vnmsubfp128,
     "vnmsubfp128",
     VX128(5, 336),
     VX128_MASK,
     {VD128, VA128, VB128, VS128},
     ""},
    ...
    {std_dbcz128,
     ...
     XRT_MASK,
     {RA, RB},
     "Data Cache Block set to Zero (1)"},
    ...
    {std_mtspr,
     ...
     0xFC0007FE,
     {SPR, RS},
     "Move to sprg, "}, /// XFX macro didnt work just put opcode + mask manually
    ...
    //	{	std_svc,			"svc",			SC(17, 0, 0),
    // SC_MASK,		{ SVC_LEV, FL1, FL2 },					"Synchronize"	},
    //	{	std_svcl,			"svcl",			SC(17, 0, 1),
    // SC_MASK,		{ SVC_LEV, FL1, FL2 },					"Synchronize"	},
    //	{	std_svca,			"svca",			SC(17, 1, 0),
    // SC_MASK,		{ SV },
    //"Synchronize"	}, 	{	std_svcla,			"svcla",
    // SC(17, 1, 1),		SC_MASK,		{ SV }, "Synchronize"	},
    ...
    {spec_02002000,
     ...
     0xffffffff,
     {0},
     "Unknown instruction - included to allow conversion to code"},
...
int PluginAnalyse(insn_t & cmd) {
    ...
    if ((opBytes == OP(0)) || (opBytes == OP(4)) || (opBytes == OP(5)) || (opBytes == OP(6)) ||
        (opBytes == OP(17)) || (opBytes == OP(31)) ||
        // gekko specific
        (opBytes == OP(56)) || (opBytes == OP(57)) || (opBytes == OP(60)) || (opBytes == OP(61))) {
        ...
        for (int opcodeLoop = 0; opcodeLoop < opcodeArraySize; opcodeLoop++) {
            ...
            if ((codeBytes & pCurrentOpcode->mask) == pCurrentOpcode->opcode) {
                ...
                while ((pCurrentOpcode->operands[operandLoop] != 0) &&
                    ...
                    int rawBits =
                        (codeBytes >> pCurrentOperand->shift) & ((1 << pCurrentOperand->bits) - 1);
                    int extendedBits =
                        (rawBits << (32 - pCurrentOperand->bits)) >> (32 - pCurrentOperand->bits);
                    ...
                    case VA128: {
                        operandData->type = o_reg;
                        operandData->reg  = ((codeBytes >> 16) & 0x1F) | (codeBytes & 0x20) |
                                           ((codeBytes >> 4) & 0x40);
                        operandData->specflag1 = 0x01; // Mark the register as being an Altivec one.
                        break;
                    }
                    ...
                    case FD: // FS
                        // case	FS:
                        {
                            operandData->type = o_reg;
                            operandData->reg  = rawBits;
                            operandData->specflag1 =
                                0x08; // Mark the register as being a Gekko one.
                            break;
                        }
 ...
 *
 *	DESCRIPTION		This callback is responsible for distributing work associated with
 *each intercepted event that we deal with. In our case we deal with the following event
 *identifiers.
 *
 *					custom_ana		:	Analyses a command (in
 *'cmd') to see if it is an Altivec instruction. If so, then it extracts information from the opcode
 *in order to determine which opcode it is, along with data relating to any used operands.
 *
 *					custom_mnem		:	Generates the mnemonic for
 *our Altivec instructions, by looking into our array of opcode information structures.
 *
 *					custom_outop	:	Outputs operands for Altivec
 *instructions. In our case, we have an alternate register set (vr0 to vr31), so our operands may be
 *marked as being Altivec registers.
 *
 *					may_be_func		:	It's perfectly OK for an
 *Altivec instruction to be the start of a function, so I figured I should return 100 here. The
 *										return value is a
 *percentage probability..
 *
 *					is_sane_insn	:	All our Altivec instructions (well,
 *the ones we've identified inside custom_ana processing), are ok.
 *
 ***************************************************************************************************/
 ...
 *
 *	DESCRIPTION		IDA will call this function only once. If this function returns
 *PLUGIN_SKIP, IDA will never load it again. If it returns PLUGIN_OK, IDA will unload the plugin but
 *remember that the plugin agreed to work with the database. The plugin will be loaded again if the
 *user invokes it by pressing the hotkey or selecting it from the menu. After the second load, the
 *plugin will stay in memory.
 *
 *	NOTES			In our Altivec case, we just hook into IDA'S callbacks if we need to
 *be active on plugin load.
 *
 ***************************************************************************************************/
