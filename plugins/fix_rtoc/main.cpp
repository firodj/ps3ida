...
plugmod_t *idaapi PluginStartup() {
...
void ProcessFunction(func_t *p_func, ea_t rtoc_ea, const unsigned long *in_gpr = NULL,
                     const bool *in_act = NULL);
void ProcessFunction(ea_t start_ea, ea_t end_ea, ea_t rtoc_ea, const unsigned long *in_gpr = NULL,
                     const bool *in_act = NULL);
...
#endif
{
    ...
    // for(ea_t seg_ea=seg_start; seg_ea<seg_end; seg_ea+=8)
    {
        ...
        if (!p_func || p_func->start_ea != func_ea) {
            if (p_func) {
        ...
        if (p_func) {
            ...
            msg("[%08d/%08zu] Processing Function - [%p - %p]...", get_func_num(func_ea),
                get_func_qty(), (void *)(uintptr_t)p_func->start_ea,
                (void *)(uintptr_t)p_func->end_ea);
            // process the current function
...
void ProcessFunction(func_t *p_func, ea_t rtoc_ea, const unsigned long *in_gpr,
                     const bool *in_act) {
    ProcessFunction(p_func->start_ea, p_func->end_ea, rtoc_ea, in_gpr, in_act);
}
void ProcessFunction(ea_t start_ea, ea_t end_ea, ea_t rtoc_ea, const unsigned long *in_gpr,
                     const bool *in_act) {
    ...
    for (ea_t ea = start; ea < end; ea += 4) {
        ...
        for (; iOp < UA_MAXOP; ++iOp) {
            ...
            case o_displ: {
                reg_used =
                    (reg < (GPR_COUNT + FPR_COUNT)) && (reg_used || ((reg == 2) || g_act[reg]));
            } break;
            case o_far:
            case o_near: {
                switch (l_cmd.itype) {
...
plugin_t PLUGIN = {
    ...
    G_PLUGIN_HELP,    // multiline help about the plugin
    G_PLUGIN_NAME,    // the preferred short name of the plugin, used by menu system
    G_PLUGIN_HOTKEY   // the preferred hotkey to run the plugin
};
